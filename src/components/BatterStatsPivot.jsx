...
const BatterStatsPivot = ({ batterStats }) => {
  ...
  const columns = [
    ...
    { key: 'avg', label: '타율', type: 'number' },
    { key: 'ops', label: 'OPS', type: 'number' },
    { key: 'games', label: '경기', type: 'number' },
    { key: 'pa', label: '타석', type: 'number' },
    { key: 'ab', label: '타수', type: 'number' },
    { key: 'r', label: '득점', type: 'number' },
    { key: 'h', label: '안타', type: 'number' },
    { key: 'hr', label: '홈런', type: 'number' },
    { key: 'rbi', label: '타점', type: 'number' },
    { key: 'slg', label: '장타율', type: 'number' },
    { key: 'obp', label: '출루율', type: 'number' },
    { key: 'bb', label: '볼넷', type: 'number' },
    { key: 'so', label: '삼진', type: 'number' },
    { key: 'sb', label: '도루', type: 'number' }
  ];
  ...
  const processedData = useMemo(() => {
    let filtered = batterStats.filter(stat => 
    ...
    return filtered;
  }, [batterStats, sortKey, sortOrder, searchTerm]);
  ...
  const downloadCSV = () => {
    ...
    link.download = `타자성적_${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
  };
  ...
  return (
    <div className="space-y-4">
      ...
      {/* 통계 요약 */}
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg shadow-sm p-4">
      ...
      <div className="text-sm text-gray-500 text-center">
        전체 {batterStats.length}명 중 {processedData.length}명 표시
      </div>
    </div>
  );
};

export default BatterStatsPivot;
